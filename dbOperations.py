...
class DbConnection():
    ...
    def addToTable(self, table: str, data: dict) -> str:
        """Insert a record (row) to the according to a dictionary containing field names (columns) as keys and values
        ...
        Returns:
            str: information about successfull operation or and error message    
        """
         
        # Muodostetaan lista sarakkeiden (kentien) nimistä ja arvoista SQL lausetta varten
        Keys = data.keys() # Luetaan sankirjan avaimet
        columnList = [] # Alustetaan saraklista tyhjäksi
        valuesList = [] # Alustetaan arvolista tyhjäksi
        ...
        try:
            ...
            sqlClause = f'INSERT INTO {table} ({columns}) VALUES ({valuesList})'
            print(sqlClause)
